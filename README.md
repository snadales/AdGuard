...
Este repositorio contiene la configuración altamente optimizada para desplegar **AdGuard Home** en una ZBook (Ubuntu Server) utilizando Docker Compose. Proporciona resolución DNS de ultra baja latencia y filtrado de contenido a nivel de red para todo el hogar.
...
* **Cero Latencia (Overhead Eliminado):** Al amarrarse directamente a la interfaz física de ZBook, el tráfico DNS evita el puente virtual de Docker (`bridge`) y las reglas de traducción de direcciones (`NAT/IPTables`). Esto ahorra entre **0.5 y 2.5 ms** por consulta, garantizando respuestas instantáneas.
...
Dado que AdGuard Home se adueña del puerto **53** de ZBook, es indispensable desactivar el servicio DNS nativo de Ubuntu (`systemd-resolved`) si está activo, para evitar conflictos de puerto:
